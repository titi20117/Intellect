...
namespace Intellect.Main
{
    class SyntacticAnalysis
    {
        ...
        public SyntacticAnalysis(string morphSentenceSpeech, string[] subsSentence)
        {
            syntacticMorphoSentenceSpeech = morphSentenceSpeech;
            syntacticSubSentence = subsSentence;
        ...
        {
            int suppleCount = syntacticMorphoSentenceSpeech.Length - syntacticMorphoSentenceSpeech.Replace("9", "").Length;
            Console.WriteLine("Количество дополнений: " + suppleCount);
            for (int i = 0; i < syntacticMorphoSentenceSpeech.Length; i++)
            {
                if (syntacticMorphoSentenceSpeech[i] == '9')
                {
        ...
        {
            string[,] clone = new string[(originMatrix.GetUpperBound(0) + 1), originMatrix.GetLength(1)];
            Array.Copy(originMatrix, 0, clone, 0, originMatrix.GetLength(1));
            //for (int i = 0; i < originMatrix.GetUpperBound(0) + 1; i++)
            //{
            //    for(int j = 0; j < originMatrix.GetLength(1); i++)
            //    {
            //        //originMatrix.CopyTo(clone, 0);
            //        Array.Copy(originMatrix, 0, clone, 0, originMatrix.GetLength(1));
            //    }
            //}
            return clone;
